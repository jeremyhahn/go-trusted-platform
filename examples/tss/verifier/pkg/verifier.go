...
import (
	...
	pb "github.com/jeremyhahn/go-trusted-platform/examples/tss/verifier/pkg/proto"
...
var (
	...
	fInsecurePort    = flag.Int("port", 8090, "The insecure port number to listen on")
	fTLSPort         = flag.Int("tls-port", 8091, "The secure port number to listen on")
	fPIN             = flag.String("pin", "123456", "User PIN")
	fSOPIN           = flag.String("so-pin", "123456", "Security Officer PIN")
	fInitialize      = flag.Bool("init", false, "True to initialize the platform")
)
...
type Verifier interface {
	...
	IssueCertificate(ekCert *x509.Certificate, keyAttrs *keystore.KeyAttributes, akPubBytes []byte) error
	Quote(ekCert *x509.Certificate, akAttrs *keystore.KeyAttributes, akPubBytes []byte) (tpm2.Quote, []byte, error)
	VerifyQuote(
		akAttrs *keystore.KeyAttributes,
...
func main() {
	...
	initParams := app.AppInitParams{
		CADir:        "trusted-data/ca",
		ConfigDir:    "trusted-data/etc",
		Debug:        true,
		DebugSecrets: true,
		Env:          "yaml",
	...
	app.AttestationConfig.InsecurePort = *fInsecurePort
	app.AttestationConfig.TLSPort = *fTLSPort
...
func (verifier *Verification) Quote(
	...
	if verifier.config.AllowOpenEnrollment {
		...
		if err := verifier.IssueCertificate(ekCert, akAttrs, akPub); err != nil {
			return tpm2.Quote{}, nil, err
		}
	}
...
func (verifier *Verification) IssueCertificate(
	...
	certReq.PermanentID = ekCert.SerialNumber.String()
	certReq.ProdModel = verifier.app.TPMConfig.IDevID.Model
	certReq.ProdSerial = verifier.app.TPMConfig.IDevID.Serial
...
func (verifier *Verification) Attest() error {
	...
	quote, nonce, err := verifier.Quote(ekCert, akAttrs, akProfile.AKPub)
	if err != nil {
		verifier.logger.Error(err)
		return err
	}
