...
func (s *RestService) NewAccountHandler(w http.ResponseWriter, r *http.Request) {
	...
	for i, contact := range req.Contact {
		...
		contacts[i] = email
		if i == 0 {
			accountID = util.NewID([]byte(email))
		}
	}
	...
	if err == nil {
		...
		if req.OnlyReturnExisting {
			...
			writeError(w, acme.AccountExistsError("Account already exists", s.accountURL(account)))
			return
		}
	} else {
		if req.OnlyReturnExisting {
	...
	key, err := s.keySerializer.Serialize(publicKey)
	if err != nil {
