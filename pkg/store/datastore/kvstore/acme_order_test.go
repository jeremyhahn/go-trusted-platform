...
func TestGetByOrderID(t *testing.T) {
	...
	for _, serializer := range serializers {
		...
		pageResult, err := orderDAO.GetByAccountID(order.AccountID, datastore.CONSISTENCY_LOCAL)
		assert.Nil(t, err)
		assert.Equal(t, 2, len(pageResult.Entities))

		// Retrieve the order
		persisted, err := orderDAO.Get(order.ID, datastore.CONSISTENCY_LOCAL)
		assert.Nil(t, err)
		...
		_, err = orderDAO.Get(order.ID, datastore.CONSISTENCY_LOCAL)
		assert.NotNil(t, err)
		assert.True(t, errors.Is(err, datastore.ErrRecordNotFound))

		_, err = orderDAO.Get(order2.ID, datastore.CONSISTENCY_LOCAL)
		assert.NotNil(t, err)
		assert.True(t, errors.Is(err, datastore.ErrRecordNotFound))
	}
}
